...
<?php
	...
	function process_posts( $x = 0 ) {
		...
		if ( $x >= count( $this->posts ) ) {
			...
			foreach ( $this->posts as $post ) {
				...
				if ( ! empty( $post['post_id'] ) ) {
					$this->processed_posts[intval($post['post_id'])] = (int) $post['post_id'];
				}
			}
		...
		foreach ( $this->posts as $post ) {
			...
			if ( $post['status'] == 'auto-draft' || $post['post_type'] == 'attachment' || $post['post_type'] == 'nav_menu_item' || $post['post_title'] == 'Sample Page' ) {
				continue;
			}
