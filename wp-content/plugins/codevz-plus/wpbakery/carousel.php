...
class Codevz_WPBakery_carousel {
	...
	public function out( $atts, $content = '' ) {
		...
		$slick = array(
			...
			'prevArrow'			=> '<button type="button" class="slick-prev"><i class="' . $atts['prev_icon'] . '"></i></button>',
			'nextArrow'			=> '<button type="button" class="slick-next"><i class="' . $atts['next_icon'] . '"></i></button>',
			'responsive'		=> array(
  				array(
					'breakpoint'	=> 769,
					'settings'		=> array(
		...
		// Out
		$out = '<div id="' . $atts['id'] . '"' . Codevz_Plus::classes( $atts, $classes ) . ' data-slick=\'' . json_encode( $slick ) . '\'>' . do_shortcode( $content ) . '</div><div' . Codevz_Plus::data_stlye( $css, $css_t, $css_m ) . '></div>';
