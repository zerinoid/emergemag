...
class Codevz_WPBakery_video_popup {
	...
	public function out( $atts, $content = '' ) {
		...
		$classes[] = $atts['inline'] ? 'cz_video_inline' : '';
		$lightbox = $atts['inline'] ? 'cz_no_lightbox' : 'cz_lightbox';
		...
		// Out
		$out = '<div id="' . $atts['id'] . '"' . Codevz_Plus::classes( $atts, $classes ) . Codevz_Plus::data_stlye( $css, $css_t, $css_m ) . '><div' . Codevz_Plus::tilt( $atts ) . '><div class="cz_vp_c"><a href="' . $atts['video'] . '" class="' . $lightbox . '"' . $data_html . '>' . $image . '<div class="cz_vp_inner"><span><i class="' . $atts['icon'] . '"></i></span>' . $atts['title'] . '</div></a></div></div></div>' . $content_html;

		return Codevz_Plus::_out( $atts, $out, array( 'tilt', 'inline_video' ) );
	}
}